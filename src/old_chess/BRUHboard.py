...
class Board:
    def __init__(self, fen = STARTING_FEN):
        ...
        self.cached_boards = {} # used to store the zobrist hash of the board after a move is made (used in the undo_move method)
        self.cached_moves = {}
        self.cached_legal_moves = {}
        self.cached_checks = {}
        self.cached_checkmates = {}
        self.cached_stalemates = {}
        self.cached_game_over = {}
        self.cached_board_values = {}
    ...
    def generate_moves(self, turn): # add caching to this with zobrist hash
        ...
        hash_key = self.zobrist_hash
        if hash_key in self.cached_moves:
            if turn in self.cached_moves[hash_key]:
                return self.cached_moves[hash_key][turn]
        else:
            self.cached_moves[hash_key] = {}
        ...
        self.cached_moves[hash_key][turn] = moves
    ...
    def is_check(self, turn): # add caching to this with zobrist hash
        ...
        '''
        hash_key = self.zobrist_hash
        if hash_key in self.cached_checks:
            return self.cached_checks[hash_key]
        ...
        for move in enemy_moves:
            ...
            if end == king_square:
                self.cached_checks[hash_key] = True
                return True
            
        self.cached_checks[hash_key] = False
        return False
    ...
    def is_game_over(self): # add caching to this with zobrist hash
        ...
        '''
        hash_key = self.zobrist_hash
        if hash_key in self.cached_game_over:
            return self.cached_game_over[hash_key]
        
        game_over = self.is_checkmate(True) or self.is_checkmate(False) or self.is_draw()
        self.cached_game_over[hash_key] = game_over
        return game_over
    ...
    def generate_legal_moves(self, turn):
        ...
        Turn is the color of the player that is making the move.'''
        hash_key = self.zobrist_hash
        if hash_key in self.cached_legal_moves:
            return self.cached_legal_moves[hash_key]
        ...
        self.cached_legal_moves[hash_key] = legal_moves
        return legal_moves
